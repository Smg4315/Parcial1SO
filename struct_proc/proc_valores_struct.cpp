...
std::map<std::string, const Persona*> personaMasLongevaPorCiudad(const std::vector<Persona>& personas) {
...
const Persona* mayorPatrimonio(const std::vector<Persona>& personas) {
    auto it = std::max_element(personas.begin(), personas.end(),
        [](const Persona& a, const Persona& b){
...
std::map<std::string, const Persona*> mayorPatrimonioPorCiudad(const std::vector<Persona>& personas) {
    std::map<std::string, const Persona*> resultado;
...
std::map<char, const Persona*> mayorPatrimonioPorGrupo(const std::vector<Persona>& personas) {
...
std::map<char, std::vector<const Persona*>> declarantesPorCalendario(const std::vector<Persona>& personas) {
    std::map<char, std::vector<const Persona*>> resultado;
    for (const auto& p : personas) {
        if (p.declaranteRenta) {
            resultado[p.calendario].push_back(&p);
        }
    }
    return resultado;
}

bool validarAsignacion(const std::vector<Persona>& personas) {
    int incorrectos = 0;
    for (const auto& p : personas) {
...
std::pair<std::string, int> ciudadMayorMenoresDe18(const std::vector<Persona>& personas) {
...
std::map<char, double> promedioIngresosPorGrupo(const std::vector<Persona>& personas) {
...
std::map<char, double> porcentajeMayores50PorCalendario(const std::vector<Persona>& personas) {
...
int main() {
    ...
    double tiempoGen = monitor.detener_tiempo();
    long memGen = monitor.obtener_memoria();
    ...
    double tiempoProc = monitor.detener_tiempo();
    long memProc = monitor.obtener_memoria();
